...
//We first install the files this is dependant on, then the methods we want accessible in this file.
/*
GP;\r c_base.run;
*/
...
/*
GP;addhelp(bqf, "This package deals with binary quadratic forms with integer coefficients. A homogeneous binary quadratic form Ax^2+Bxy+Cy^2 is stored as [A,B,C]. A proper discriminant is an integer that is equivalent to 0 or 1 modulo 4 and is not a square. \n Subtopics:\n Discriminants (disc)\n Basic operations (bqfbasic)\n Indefinite forms (ibqf)\n Class group and composition (bqfclass)\n Representation of numbers (bqfsolve)");
GP;addhelp(disc,"disclist, discprimeindex, fdisc, isdisc, pell, posreg, quadroot.");
GP;addhelp(bqfbasic,"bqf_automorph, bqf_disc, bqf_isequiv, bqf_isreduced, bqf_random, bqf_random_D, bqf_red, bqf_roots, bqf_trans, bqf_trans_coprime, ideal_tobqf.");
GP;addhelp(ibqf,"ibqf_isrecip, ibqf_leftnbr, ibqf_redorbit, ibqf_rightnbr, ibqf_river, ibqf_riverforms, ibqf_symmetricarc, mat_toibqf.");
GP;addhelp(bqfclass,"bqf_comp, bqf_ncgp, bqf_ncgp_lexic, bqf_pow, bqf_square.");
GP;addhelp(bqfsolve,"bqf_bigreps, bqf_linearsolve, bqf_reps.");
*/
...
//DEBUGGER
/*
GP;install("bqf_debug","D0,G,D0,G,D0,G,D0,G,D0,G,D0,G,D0,G,p","bqf_debug","./c_bqf.so");
*/
//This method helps debug the methods here
GEN bqf_debug(GEN a, GEN b, GEN c, GEN d, GEN e, GEN f, GEN g, long prec){
  GEN rootD=gsqrt(bqf_disc(a),prec);
  return ibqf_river_positions_forms(a,rootD);
}
...
GEN quadroot(GEN D){
  pari_sp top = avma;
  if(smodis(D,2)==0) return gerepileupto(top,gmulsg(2,quadgen0(D, -1)));
  return gerepileupto(top,gsubgs(gmulsg(2,quadgen0(D, -1)),1));
}

//quadroot but does type check.
GEN quadroot_typecheck(GEN D){
    if(isdisc(D)==0) pari_err_TYPE("Not a discriminant",D);
    return quadroot(D);
}
...
GEN dbqf_reps(GEN qred, GEN D, GEN n, int proper, int half){
  ...
  for(long i=1;i<lg(divs);++i){
	...
	if(signe(n)==-1) dbqf_reps_proper(qred,D,negi(gel(divs,i)),&sols,&nsols,f,&terminate);
	else dbqf_reps_proper(qred,D,gel(divs,i),&sols,&nsols,f,&terminate);//Doing the proper solve
